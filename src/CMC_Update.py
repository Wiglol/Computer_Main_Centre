...
def cmc_update_status_check(
    cmc_folder: Path,
    repo: str = DEFAULT_REPO,
    branch: str = DEFAULT_BRANCH,
) -> str:
    """
    Returns "up_to_date" | "update_available" | "diverged" | "unknown".
    Works for both git clones AND plain zip installs.

    Strategy:
      1. If git is available and .git exists → use git fetch/rev-parse (fast, no API rate limit).
      2. Otherwise: load installed_sha from state file.
         If missing, try to seed it from UpdateNotes/VERSION.txt (baked into the repo zip).
         Then compare against GitHub API latest SHA.
    """
    ...
    if not latest:
        return "unknown"  # no internet / rate limited
...
def cmc_update_check(
    ...
    repo: str = DEFAULT_REPO,
    branch: str = DEFAULT_BRANCH,
    cmc_folder: Path | None = None,
) -> None:
    """Check for updates and print result. Works for git clones and zip installs."""
    state = _load_state()
    installed = state.get("installed_sha")
    ...
    if installed:
        ...
        p("ℹ️ No installed version recorded. Run: cmc update to install.")
